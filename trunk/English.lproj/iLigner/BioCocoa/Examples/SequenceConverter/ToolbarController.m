...
-(void)awakeFromNib
{
 ...
 addToolbarItem(toolbarItems,@"Export",@"Export Sequences",@"Export Sequences",@"Export data to a sequence file",myDoc,@selector(setImage:),[NSImage imageNamed:@"export.tiff"],@selector(saveFile:),NULL);

 addToolbarItem(toolbarItems,@"Import",@"Import Sequences",@"Import Sequences",@"Import data from a sequence file",myDoc,@selector(setImage:),[NSImage imageNamed:@"import.tiff"],@selector(chooseTextFile:),NULL);
...
{
    return [NSArray arrayWithObjects:@"Import",@"Export",nil];
}
